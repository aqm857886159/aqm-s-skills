...
def fetch_arxiv(query: str, max_results: int, timeout: float = 20.0) -> list[dict[str, object]]:
...
def main() -> int:
    ...
    if args.fixture:
        ...
        items = fetch_arxiv(args.query, args.max_results)
    else:
        parser.error("provide a query or --fixture")
    if args.since:
        items = filter_recent(items, date.fromisoformat(args.since))
    payload = {"query": args.query, "count": len(items), "papers": items}
