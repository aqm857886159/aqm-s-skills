...
| [`wechat-chat-export`](skills/wechat-chat-export/) | “把我本机微信里这个群从 8 月 1 日起的记录真正导出来” | 对当前数据库做就绪验证，再按群/时间范围导出私有结构化 JSON |
...
网络 Skill 默认只访问公开来源。`reference-video-deconstruction` 默认只在本地使用 FFmpeg，不上传视频。`wechat-chat-export` 只消费用户已有的本地 per-database key，默认打码发送者、拒绝写入 Git 工作区，也不会取钥、修改微信、上传记录或发送消息。
...
- 已有、用户授权的微信 per-database key bundle：只有 `wechat-chat-export` 需要；公开 Skill 不负责首次取钥。
